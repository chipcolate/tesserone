...
/// true = bar / black, false = space / white. Run-length merged for crisp edges.
struct OneDimBarcodeView: View {
    ...
    private func fillRun(_ start: Int, end: Int, unit: CGFloat, height: CGFloat, in context: GraphicsContext) {
        let rect = CGRect(
            x: CGFloat(start) * unit,
            y: 0,
            width: CGFloat(end - start) * unit,
            height: height
        )
